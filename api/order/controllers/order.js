...
/**
 * Given a rupiah amount, return amount in cents
 * @param {number} number 
 */
...
module.exports = {
    ...
    async create(ctx) {
        ...
        if (!product) {
            return ctx.throw(400, 'Silahkan pilih produk dahulu')
        }

        const realProduct = await strapi.services.product.findOne({ id: product.id })
        if (!realProduct) {
            return ctx.throw(404, 'Tidak ada produk dengan id tersebut')
        }
    ...
    async confirm(ctx) {
        ...
        if (session.payment_status === 'paid') {
            ...
            ctx.throw(400, "Pembayaran gagal, silahkan hubungi admin")
        }
    }
};
